...
from .models import Ebookbuy  # 將models.py 中的class匯入

#分頁要用的函式庫
from django.core.paginator import Paginator,EmptyPage,PageNotAnInteger

def ebookbuy(request):
    
    # 抓資料庫中的所有資料 objects => 資料
    # order_by  排序 依 欄位名稱 id 來做排序
    #  - 遞減 若沒有加 - 號時，則為遞增
    data = Ebookbuy.objects.all().order_by('-id')
    
    paginator = Paginator(data,25)  # 24筆為一頁
    ...
    #先採用字典方式傳送至網頁
    content = {"ebookbuyList":pageData}
    
    #render(request,網頁名稱,參數內容)
    return render(request,'ebookbuy.html',content)