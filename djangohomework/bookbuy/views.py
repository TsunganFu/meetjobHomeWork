...
from .models import Bookbuy  # 將models.py 中的class匯入
...
def bookbuy(request):
    ...
    #  - 遞減 若沒有加 - 號時，則為遞增
    data = Bookbuy.objects.all().order_by('-id')
    ...
    #先採用字典方式傳送至網頁
    content = {"bookbuyList":pageData}
    
    #render(request,網頁名稱,參數內容)
    return render(request,'bookbuy.html',content)